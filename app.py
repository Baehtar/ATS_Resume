    ...
    with col_form:
        ...
        with st.expander("💼 Work / Internship Experience", expanded=False):
            ...
            for i, exp in enumerate(exp_list):
                ...
                gen_col, _ = st.columns([0.7, 0.3])
                with gen_col:
                    if st.button("Generate 2-6 Bullets (AI)", key=f"gen_entry_{i}"):
                        with st.spinner("Generating bullets for this entry..."):
                            entry_info = {
                                "company": exp.get("company", ""),
                                "role": exp.get("role", ""),
                                "domain": st.session_state.resume["personal"].get("headline", ""),
                                "client": exp.get("company", ""),
                                "details": "\n".join([b for b in exp.get("bullets", []) if b.strip()]) or "",
                                "tools": st.session_state.resume["personal"].get("headline", ""),
                                "years": ""
                            }
                            gen = resume_generator.generate_entry_bullets(entry_info)
                            if gen.get("bullets"):
                                exp["bullets"] = gen["bullets"]
                                st.session_state.resume["experience"] = exp_list
                                if gen.get("api_used"):
                                ...
                                st.error("Could not generate bullets for this entry.")
                        st.rerun()
